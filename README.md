...
以下为系统内部通信结构，源自实际部署配置：

| 端口       | 方向             | 角色                          | 内容                                      | 参与者                     |
|------------|------------------|-------------------------------|-------------------------------------------|---------------------------- |
| 7000       | 服务器 → 前端机 | **101 监听 (TCP Server)**     | 开始检测信号 `'o'` + 卷号               | 仅 101*                    |
| 19800      | 服务器 → 前端机 | **101 监听 (TCP Server)**     | 分割定位参数                              | 仅 101*                    |
| 19300      | 前端机 → 服务器 | **所有前端机主动连接 (TCP Client)** | 特征 `'F'` / 状态 `'T'`                 | 101 + 所有其他前端机       |
| 19700      | 101 → 服务器    | **101 主动连接 (TCP Client)** | 卷长/宽/速度/卷号/状态（服务器界面显示） | 仅 101*                    |
...
> - 多相机模式下，每个相机可视为一个前端机，共享同一服务器控制逻辑。
> - 这里的"仅101"表示到是101作为前端机到主节点统一了前端机和服务器之间到通讯，实际上所有前端机都要发布自身状态到101上统一管理,同理服务器发送的请求也由101统一发送给所有到其他前端机
...
- **Redis PUB/SUB**: 用于在主前端机（101）和其他前端机之间传递控制信号和状态更新
- **Redis 键值存储**: 用于存储共享配置和临时状态信息
- **主控节点（101）**: 负责发布控制信号并聚合遥测数据
- **从属节点**: 接收控制信号并发送遥测数据

> 注意：在未来的多相机模式下（所有相机在同一主机上），将不再需要 Redis 进行跨主机同步，因为所有组件都在同一台机器上运行。
---

## 动态算法库设计（未来规划）

为了提高系统的灵活性和可维护性，我们计划实现动态算法库机制：

- **插件化算法架构**: 所有算法类将通过统一接口进行规约，实现算法的动态加载和替换
- **动态库支持**: 算法将以动态库（DLL/.so）形式存在，支持运行时加载
- **远程更新能力**: 服务器可推送新的算法库到前端机，无需重新部署整个应用
- **现场优化**: 前端机可自行替换表现不佳的算法，进行针对性优化
- **元数据管理**: 每个算法包含元信息（参数列表、处理步骤、版本等），便于管理和验证
- **热插拔**: 支持在不停机的情况下动态替换算法库

这种设计将允许我们在不远程访问客户主机重新编译程序的情况下，动态替换某些表现不佳的现场算法，提高系统的可维护性和适应性。
...
我们欢迎社区贡献！如果您希望为项目做出贡献，请查看 [CONTRIBUTING.md](file:///d:/codespace/CFP/CONTRIBUTING.md) 了解我们的分支策略、代码规范和开发流程。
